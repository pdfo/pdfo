...
%               lb <= x <= ub.
%
%   In the backend, LINCOA calls the late Professor M.J.D. Powell's Fortran code
%   with the same name.
%
...
%       of the approximate solution obtained by LINCOA; radius_final should be
...
elseif ~strcmp(invoker, 'pdfo') &&  probinfo.feasibility_problem
    ...
    % Consistently, npt <= maxnpt guarantees nw <= maxint.
    if npt > maxnpt
        npt = maxnpt;
